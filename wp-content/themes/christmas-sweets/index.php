	...
	<main id="main" class="site-main" role="main">
		...
		if ( have_posts() ) {
			if ( is_archive() ) {
				...
				<header class="page-header">
					<h1 class="entry-title">
						...
						printf( esc_html__( 'Search Results for: %s', 'christmas-sweets' ), '<span>' . get_search_query() . '</span>' );
						?>
					</h1>
				</header><!-- .page-header -->
				<?php
			...
			if ( is_home() && current_user_can( 'publish_posts' ) ) {
				...
				<p>
				<?php
				/* translators: 1: link to the admin area where a new post can be created. */
