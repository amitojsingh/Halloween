<?php
/**
 * The template for Update Notification
 *
 * @package Christmas_Bell
 */

if ( ! class_exists('CT_Update_Checker') ):

/**
 * A update checker class.
 *
 */
class CT_Update_Checker {
	...
	public function requestUpdate($queryArgs = array()){
		...
		if ( ($code == 200) && !empty($body) ){
			$themeUpdate = catchthemes_update::fromJson($body);
...
if ( !class_exists('catchthemes_update') ):
...
$CT_Update_Checker = new CT_Update_Checker(
   'christmas-bell',
   'https://catchthemes.com/update-notifier/christmas-bell/theme_meta_data.json'
);
