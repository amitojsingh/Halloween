...
if ( ! function_exists( 'christmasbell_breadcrumb' ) ) :
	function christmasbell_breadcrumb() {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'christmas-bell' ); // text for the 'Home' link
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'christmas-bell' ); // text for a category page
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'christmas-bell' ); // text for a search results page
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'christmas-bell' ); // text for a tag page
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'christmas-bell' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'christmas-bell' ); // text for the 404 page
		...
		if ( ! is_front_page() ) {
			...
			elseif ( is_day() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'christmas-bell' ) ) ) ), get_the_time( __( 'Y', 'christmas-bell' ) ) ) ;
				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'christmas-bell' ) ), get_the_time( __( 'm', 'christmas-bell' ) ) ) ), get_the_time( __( 'F', 'christmas-bell' ) ) );
				echo $before . get_the_time( __( 'd', 'christmas-bell' ) ) . $after;

			}
			elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'christmas-bell' ) ) ) ), get_the_time( __( 'Y', 'christmas-bell' ) ) ) ;
				echo $before . get_the_time( __( 'F', 'christmas-bell' ) ) . $after;

			}
			elseif ( is_year() ) {
				echo $before . get_the_time( __( 'Y', 'christmas-bell' ) ) . $after;
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'christmas-bell' ), max( $paged, $page ) );
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
	...
	} // end christmasbell_breadcrumb_lists
endif;
