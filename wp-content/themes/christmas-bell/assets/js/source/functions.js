...
( function( $ ) {
    ...
    $( function() {
        $( document ).ready( function() {
            $('.featured-content-wrapper .entry-container').matchHeight();
        });
    });
    ...
    ( function() {
        ...
        menuToggle.on( 'click.christmasbell', function() {
    ...
    ( function() {
        ...
        function toggleFocusClassTouchScreen() {
            if ( window.innerWidth >= 910 ) {
                $( document.body ).on( 'touchstart.christmasbell', function( e ) {
                    if ( ! $( e.target ).closest( '.main-navigation li' ).length ) {
                        $( '.main-navigation li' ).removeClass( 'focus' );
                    }
                } );
                siteNavigation.find( '.menu-item-has-children > a' ).on( 'touchstart.christmasbell', function( e ) {
                ...
                siteNavigation.find( '.menu-item-has-children > a' ).unbind( 'touchstart.christmasbell' );
            }
        }

        if ( 'ontouchstart' in window ) {
            $( window ).on( 'resize.christmasbell', toggleFocusClassTouchScreen );
            toggleFocusClassTouchScreen();
        }

        siteNavigation.find( 'a' ).on( 'focus.christmasbell blur.christmasbell', function() {
    ...
    // Fix sub-menus for touch devices and better focus for hidden submenu items for accessibility.
    ( function() {
        if ( ! siteNavigationFooter.length || ! siteNavigationFooter.children().length ) {
            return;
        }
        ...
        function toggleFocusClassTouchScreen() {
            if ( window.innerWidth >= 910 ) {
                ...
                siteNavigationFooter.find( '.menu-item-has-children > a' ).on( 'touchstart', function( e ) {
                    var el = $( this ).parent( 'li' );
                ...
                siteNavigationFooter.find( '.menu-item-has-children > a' ).unbind( 'touchstart' );
            }
        }
        ...
        siteNavigationFooter.find( 'a' ).on( 'focus blur', function() {
            $( this ).parents( '.menu-item' ).toggleClass( 'focus' );
        } );
    })();
    //Footer Menu End
